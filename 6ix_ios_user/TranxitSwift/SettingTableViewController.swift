...
class SettingTableViewController: UITableViewController {
    ...
    private let header = [Constants.string.changeLanguage, Constants.string.favourites, .Empty, Constants.string.accountManagement]
    
    private let favouriteLocation = 1
    private let changeLanguage = 0
    private let otherLocations = 2
    private let deleteAccount = 3
...
//        self.navigationItem.backBarButtonItem = UIBarButtonItem()
    ...
    private func viewwillAppearCustom() {
        self.navigationItem.leftBarButtonItem = UIBarButtonItem(image: #imageLiteral(resourceName: "back-icon").withRenderingMode(.alwaysTemplate), style: .plain, target: self, action: #selector(self.backClick))
...
extension SettingTableViewController {
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return section == self.favouriteLocation ? numberOfRows : (section == self.changeLanguage ? Language.count : (section == self.deleteAccount ? 1 : Int.removeNil(self.locationService?.others?.count)))
    }
    ...
    override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return (indexPath.section == self.favouriteLocation ? 80 : (indexPath.section == self.changeLanguage || indexPath.section == self.deleteAccount) ? 40 : 60)*(UIScreen.main.bounds.height/568)
    }
    ...
    private func didSelect(at indexPath : IndexPath) {
        ...
        }else if indexPath.section == self.deleteAccount { // DELETE ACCOUNT
            //Preparing data
            var optionsArray = [FeedbackOption]()
            optionsArray.append(FeedbackOption(optionTitle: Constants.string.deleteAccountReason1.localize()))
            optionsArray.append(FeedbackOption(optionTitle: Constants.string.deleteAccountReason2.localize()))
            optionsArray.append(FeedbackOption(optionTitle: Constants.string.deleteAccountReason3.localize()))
            optionsArray.append(FeedbackOption(optionTitle: Constants.string.deleteAccountReason4.localize()))
            
            tableView.deselectRow(at: indexPath, animated: true)
            
            self.presentFeedbackViewController(feedbackInterfaceData: FeedbackInterfaceData(mainTitle: Constants.string.deleteAccount.localize(), mainDescription: Constants.string.deleteAccountDescription.localize(), submitBtnTitle: Constants.string.delete.localize()), feedbackOptions: optionsArray) { [weak self] reason in
                self?.removeAccount(reason: reason)
            }
        }
    }
    
    private func removeAccount(reason: String){
        
        self.showInputDialog(title: Constants.string.passwordAuthentication.localize(), subtitle: Constants.string.passwordAuthenticationDesciption.localize(), actionTitle: Constants.string.delete.localize(), cancelTitle: Constants.string.Cancel.localize(), style: .destructive, inputPlaceholder: Constants.string.password.localize(), isSecureTextEntry: true, inputKeyboardType: .default, cancelHandler: nil) { [weak self] password in
            
            guard let self = self else{
                return
            }
            
            if password == nil || password?.trimmingCharacters(in: .whitespaces) == ""{
                self.showSimpleAlert(title: Constants.string.passwordRequired.localize(), message: Constants.string.passwordRequiredDescription.localize())
            }else{
                self.loader.isHidden = false
                let deleteObject = DeleteAccountModel(password: password!, reason: reason)
                self.presenter?.post(api: .deleteAccount, url: "\(Base.deleteAccount.rawValue)/\(User.main.id!)", data: deleteObject.toData())
            }
        }
    }
    ...
    private func getCell(for tableView : UITableView,at indexPath : IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == self.favouriteLocation, let tableCell = tableView.dequeueReusableCell(withIdentifier: tableCellId, for: indexPath) as? SettingTableCell {
            ...
            if #available(iOS 13.0, *) {
                tableCell.imageViewIcon.tintColor = .label
            } else {
            ...
            Common.setFont(to: tableCell.textLabel!)
            tableCell.textLabel?.numberOfLines = 0
            tableCell.textLabel?.text = Constants.string.deleteAccount.localize()
            tableCell.selectionStyle = .none
            tableCell.textLabel?.textColor = .systemRed
            return tableCell
        }
...
extension SettingTableViewController : PostViewProtocol {
    ...
    func onError(api: Base, message: String, statusCode code: Int) {
        ...
        switch api {
        case .deleteAccount:
            self.showSimpleAlert(title: Constants.string.errorDeletingAccount.localize(), message: message)
            break
        default:
            self.showSimpleAlert(title: Constants.string.error.localize(), message: message)
            break
        }
    }
    
    func deleteAccount(api: Base, data: DeleteAccountResponseModel?) {
        if data?.status ?? false{
            forceLogout()
        }else{
            self.showSimpleAlert(title: Constants.string.errorDeletingAccount.localize(), message: data?.message ?? "There is something wrong while deleting your account account")
        }
    }
