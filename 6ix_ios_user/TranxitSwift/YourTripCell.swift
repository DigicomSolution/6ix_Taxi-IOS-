...
class YourTripCell: UITableViewCell {
    ...
    func set(values : Request) {
            ...
            if image != nil {
                DispatchQueue.main.async {
                    if #available(iOS 13.0, *) {
                        image?.withTintColor(.label, renderingMode: .alwaysTemplate)
                    }
        ...
        if self.isPastButton {
            ...
            let isFlateRate = (values.payment?.isFlatRate ?? false)
            let total =  isFlateRate  ? "\((values.payment?.flat_rate ?? "0.0").double)" : "\(values.payment?.total ?? 0.0)"
//            self.labelPrice.text = currency + " \(Formatter.shared.limit(string: "\(total)", maximumDecimal: 2))"

            
            
            if let totalFloat = Float(total){
                if let tips = values.payment?.tips{
